function figfilenames = plot_tomo_and_faults7(TOMO,FAULTS,title_str,BOUNDS,OPTIONS,funprint...
    ,WELLS, elev_mean, BRADY2DGRID, SLICES, BRADYBOX...
...
% 20181119 Kurt Feigl
%  Label wells
%  move rsearch to OPTIONS structure and remove from argument list
%  Add FileNamePrefix to list of input arguments
...
if exist('OPTIONS','var') == 1
    OPTIONS
    if isfield(OPTIONS,'vmin') == 0
        OPTIONS.vmin = nan;
    end
    if isfield(OPTIONS,'vmax') == 0
        OPTIONS.vmax =nan;
    end
    ...
    if isfield(OPTIONS,'rsearch') == 1
        rsearch=OPTIONS.rsearch;
    else
        rsearch = 0.0;
    end
...
%         end
    ...
    if isfield(OPTIONS,'draw_mudpots') == 1
        draw_mudpots=OPTIONS.draw_mudpots;
    else
        draw_mudpots = 0;
    end
    if isfield(OPTIONS,'draw_fumaroles') == 1
        draw_fumaroles=OPTIONS.draw_fumaroles;
    else
        draw_fumaroles = 0;
    end
    if isfield(OPTIONS,'draw_traces') == 1
        draw_traces=OPTIONS.draw_traces;
    else
        draw_traces = 0;
    end
end

%unique(S26.ParticleNumber);
...
%     subplot(1,3,3);histogram(colvec(TOMO.Zp));xlabel('Zp [m]');ylabel('Count');
    ...
    for kslice = 1:nslices
        ...
        Q3 = colvec(squeeze(image2d));
        if numel(colvec(Q1)) > 10 && numel(colvec(Q2)) > 10 && numel(colvec(Q3)) > 10
            %image2dinterp = interp2(Q1,Q2,squeeze(image2d),M1,M2);
            fprintf(1,'Starting 2-D scatteredInterpolant at t = %.001f seconds\n',toc(tstart));
            Finterpolant = scatteredInterpolant(colvec(Q1),colvec(Q2),colvec(Q3)...
                ,interpolation_method,extrapolation_method);
            %whos Finterpolant
            ...
            if numel(image2dinterp) == numel(M1)
                ...
                % set the color table based on manual input
                if isfinite(OPTIONS.vmin) == 1 && isfinite(OPTIONS.vmax)==1
                    caxis([OPTIONS.vmin,OPTIONS.vmax]);
                    caxis manual;
                    v_contours = nice_steps([OPTIONS.vmin,OPTIONS.vmax],5);
                else
                    % set the color table bounds automatically for each
                    % slice
                    ...
                    if isfinite(vmin) == 1 && isfinite(vmax)==1
                        ...
                        caxis auto;
                        v_contours = nice_steps([-Inf,Inf],5);
                    end
                end
                    ...
                    switch knorm
                        case 1
                            plot([min(BRADYBOX.Yp),max(BRADYBOX.Yp),max(BRADYBOX.Yp),min(BRADYBOX.Yp),min(BRADYBOX.Yp)]...
                                ...
                                ,'-','color',[0.5, 0.5, 0.5]);
                            AX.XTick = [nanmin(BOUNDS.Yp):300:nanmax(BOUNDS.Yp)];
                        ...
                        case 2
                            plot([min(BRADYBOX.Xp),max(BRADYBOX.Xp),max(BRADYBOX.Xp),min(BRADYBOX.Xp),min(BRADYBOX.Xp)]...
                                ,[0               ,0.              ,max(BRADYBOX.Zp),max(BRADYBOX.Zp),0.             ]...
                               ,'-','color',[0.5, 0.5, 0.5]);
                            AX.XTick = [nanmin(BOUNDS.Xp):250:nanmax(BOUNDS.Xp)];
                            AX.YTick = [nanmin(BOUNDS.Zp):200:nanmax(BOUNDS.Zp)];
                       case 3
                            i5 = [1,2,3,4,1]; % indices to close rectangle
                            plot(BRADYBOX.Xp(i5),BRADYBOX.Yp(i5),'-','color',[0.5, 0.5, 0.5]);
                            AX.XTick = [nanmin(BOUNDS.Xp):250:nanmax(BOUNDS.Xp)];
                            AX.YTick = [nanmin(BOUNDS.Yp):300:nanmax(BOUNDS.Yp)];
                        otherwise
                            error(sprintf('unknown knorm = %d\n',knorm));
                    end
                    AX.XGrid = 'on';
                ...
                if draw_topo == 1
                     switch knorm
                        case 1
                           ...
                           patch(yrun,topo,'w');
                           plot(yrun(1:end-2),topo(1:end-2),'w-','LineWidth',3);
                         case 2                         
                           ...
                           whos topo yrun
                           patch(xrun,topo,'w');
                           plot(xrun(1:end-2),topo(1:end-2),'w-','LineWidth',3);
                        case 3
                ...
                if draw_wells == 1
                    ...
                    well_colors  = cell(nwells,1);
                    for i=1:nwells
                        if contains(WELLS.ActivityDuringPoroTomoTesting(i),'Inj') == 1
                            well_symbols{i} = 'v';  % downward pointing triangle for injection well
                            well_colors{i}  = 'b';  % blue for injection well
                        elseif contains(WELLS.ActivityDuringPoroTomoTesting(i),'Pumping') == 1
                            well_symbols{i} = '^';  % upward pointing triangle for production well
                            well_colors{i}  = 'r';  % red for production well
                        elseif contains(WELLS.ActivityDuringPoroTomoTesting(i),'Obs') == 1
                            well_symbols{i} = 'o';  % open circle for observation well
                            well_colors{i}  = 'g';  % green for observation well
                        else
                            well_symbols{i} = '+';  %
                            well_colors{i}  = 'k';  % 
                        end
                    ...
                    switch knorm
                        case 1
                            for i=1:nwells
                                if abs(WELLS.grdsurf.Xp(i)-SLICES.Xp(kslice)) < rsearch
                                    plot(WELLS.grdsurf.Yp(i),WELLS.grdsurf.Zp(i)...
                                        ,strcat(well_symbols{i},'k')...
                                        ,'MarkerSize',7,'MarkerFaceColor',well_colors{i}); % symbol
                                    text(WELLS.grdsurf.Yp(i),WELLS.grdsurf.Zp(i)+rsearch/2,char(WELLS.WellName{i})...
                                        ,'HorizontalAlignment','Left','VerticalAlignment','Middle','Rotation',90); % label
                                    ...
                                    plot([WELLS.perftop.Yp(i),WELLS.perfbtm.Yp(i)]...
                                        ...
                                        ,'k:','LineWidth',2); % thin, dotted line for perforated interval
                        ...
                        case 2
                            for i=1:nwells
                                if abs(WELLS.grdsurf.Yp(i)-SLICES.Yp(kslice)) < rsearch
                                    plot(WELLS.grdsurf.Xp(i),WELLS.grdsurf.Zp(i)...
                                        ,strcat(well_symbols{i},'k')...
                                        ,'MarkerSize',7,'MarkerFaceColor',well_colors{i}); % symbol
                                    text(WELLS.grdsurf.Xp(i),WELLS.grdsurf.Zp(i)+rsearch/2,char(WELLS.WellName{i})...
                                        ,'HorizontalAlignment','Left','VerticalAlignment','Middle','Rotation',90); % label
                        ...
                        case 3
                            % only draw wells on uppermost horizontal slice
                            if (SLICES.Zp(kslice) - nanmax(SLICES.Zp)) >= 0
                                for i=1:nwells
                                    plot(WELLS.grdsurf.Xp(i),WELLS.grdsurf.Yp(i)...
                                        ,strcat(well_symbols{i},'k')...
                                        ,'MarkerSize',7,'MarkerFaceColor',well_colors{i}); % symbol
                                    text(WELLS.grdsurf.Xp(i),WELLS.grdsurf.Yp(i),char(WELLS.WellName{i})...
                ...
                %% draw the particle traces
                if draw_traces == 1
                    iparticles = unique(TRACES.ParticleNumber);
                    nparticles = numel(iparticles);
                    switch knorm
                        case 1
                            for i=1:nparticles
                                ii = find(TRACES.ParticleNumber == i);
                                ii = intersect(ii,find(abs(TRACES.Xp-SLICES.Xp(kslice)) < 1*rsearch));
                                plot(TRACES.Yp(ii),TRACES.Zp(ii),'w-','LineWidth',1); % thin, solid line trace
                                
                            end
                       case 2
                            for i=1:nparticles
                                ii = find(TRACES.ParticleNumber == i);
                                ii = intersect(ii,find(abs(TRACES.Yp-SLICES.Yp(kslice)) < 1*rsearch));
                                plot(TRACES.Xp(ii),TRACES.Zp(ii),'w-','LineWidth',1); % thin, solid line trace
                                
                            end
                        case 3
                            for i=1:nparticles
                                ii = find(TRACES.ParticleNumber == i);
                                ii = intersect(ii,find(abs(TRACES.Zp-SLICES.Zp(kslice)) < 1*rsearch));
                                plot(TRACES.Xp(ii),TRACES.Yp(ii),'w-','LineWidth',1); % thin, solid line trace
                ...
                if draw_mudpots == 1
                    switch knorm
                        case 1
                            for i=1:numel(MUDPOTS.Xp)
                                if abs(MUDPOTS.Xp(i)-SLICES.Xp(kslice)) < 1*rsearch
                                    plot(MUDPOTS.Yp(i),MUDPOTS.Zp(i),'kd','MarkerFaceColor','w','MarkerSize',7);
                                end
                            end
                        case 2
                            for i=1:numel(MUDPOTS.Yp)
                                if abs(MUDPOTS.Yp(i)-SLICES.Yp(kslice)) < 1*rsearch
                                    plot(MUDPOTS.Xp(i),MUDPOTS.Zp(i),'kd','MarkerFaceColor','w','MarkerSize',7);
                                end
                            end
                        case 3
                            for i=1:numel(MUDPOTS.Zp)
                                if SLICES.Zp(kslice) >= max(SLICES.Zp) % only plot on uppermost slice
                ...
                if draw_fumaroles == 1
                    switch knorm
                        case 1
                            for i=1:numel(FUMAROLES.Xp)
                                if abs(FUMAROLES.Xp(i)-SLICES.Xp(kslice)) < 1*rsearch
                                    plot(FUMAROLES.Yp(i),FUMAROLES.Zp(i),'kd','MarkerFaceColor','k','MarkerSize',7);
                                end
                            end
                        case 2
                            for i=1:numel(FUMAROLES.Yp)
                                if abs(FUMAROLES.Yp(i)-SLICES.Yp(kslice)) < 1*rsearch
                                    plot(FUMAROLES.Xp(i),FUMAROLES.Zp(i),'kd','MarkerFaceColor','k','MarkerSize',7);
                                end
                            end
                        case 3
                            for i=1:numel(FUMAROLES.Zp)
                                if SLICES.Zp(kslice) >= max(SLICES.Zp) % only plot on uppermost slice
                                   plot(FUMAROLES.Xp,FUMAROLES.Yp,'kd','MarkerFaceColor','k','MarkerSize',7);
                                end
                            end
                        otherwise
                            error(sprintf('unknown knorm = %d\n',knorm));
...
%                 end
                ...
                if flattened_cube == 1
                    FileNameOut = sprintf('%s_cubeX%05.0fmY%05.0fmZ%05.0fm',title_str...
                        ,SLICES.Xp(kslice),SLICES.Yp(kslice),SLICES.Zp(kslice));
                else
                    switch knorm
                        case 1
                            FileNameOut = sprintf('%s_XnormX%05.0fm',title_str,SLICES.Xp(kslice));
                        case 2
                            FileNameOut = sprintf('%s_YnormY%05.0fm',title_str,SLICES.Yp(kslice));
                        case 3
                            FileNameOut = sprintf('%s_ZnormZ%05.0fm',title_str,SLICES.Zp(kslice));
                        otherwise
                            error(sprintf('unknown knorm %d\n',knorm));
                    end
                end
                ...
                % pre-pend prefix to file name
                FileNameOut = strcat(FileNamePrefix,FileNameOut);
                
                % call the appropriate printing function, which will append
                % file type (e.g., '.pdf')
                feval(funprint,FileNameOut);
                kfiles = kfiles+1;
                figfilenames{kfiles} = FileNameOut;
            else
                size(image2dinterp)
                size(M1)
                warning('miscount 1');
